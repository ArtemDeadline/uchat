...
int check_register(wig_log *widgets){
        ...
        if(check_pass(leng_p,leng_c,gtk_entry_get_text(GTK_ENTRY(widgets->ip_pasword)),gtk_entry_get_text(GTK_ENTRY(widgets->ip_confirm))) &&check_login_with_phone(leng_l,leng_f,leng_la)){
